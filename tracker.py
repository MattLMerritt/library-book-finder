...
class OBBProcessor:
    """Handles processing of Oriented Bounding Box (OBB) detections."""
    ...
    def extract_rotated_crop(frame, obb_points):
        ...
        try:
            ...
            return OBBProcessor._fallback_crop(frame, obb_points)
    
    @staticmethod
    def _fallback_crop(frame, obb_points):
        """
        Fallback method to extract a regular bounding box crop when OBB extraction fails.
        
        Args:
            frame (numpy.ndarray): The input frame
            obb_points (numpy.ndarray): 8-point OBB coordinates
            
        Returns:
            numpy.ndarray: Cropped image or dummy crop if extraction fails
        """
        try:
    ...
    @staticmethod
    def process_obb_detection(frame, obb_points, track_id, cls_id, model, 
                             track_ocr_confidence, extracted_texts, track_colors, 
                             reader_instance, confidence_threshold):
        """
        Processes a single OBB detection including OCR and visualization.
        
        Args:
            frame (numpy.ndarray): The video frame
            obb_points (numpy.ndarray): 8-point OBB coordinates
            track_id (int): Track ID for the detection
            cls_id (int): Class ID for the detection
            model (YOLO): The YOLO model
            track_ocr_confidence (dict): Dictionary storing OCR confidence scores
        ...
        if should_attempt_ocr:
            ...
            crop_img = OBBProcessor.extract_rotated_crop(frame, obb_points)
        ...
        # --- VISUALIZATION ---
        OBBProcessor.draw_obb_visualization(frame, obb_points, track_id, cls_id, model, track_colors)
    
    @staticmethod
    def draw_obb_visualization(frame, obb_points, track_id, cls_id, model, track_colors):
        """
        Draws visualization for OBB detection on the frame.
...
class RegularBoxProcessor:
    """Handles processing of regular bounding box detections."""
    
    @staticmethod
    def extract_regular_crop(frame, box):
        """
        Extracts a regular crop from the frame using bounding box coordinates.
    ...
    @staticmethod
    def process_regular_detection(frame, box, track_id, cls_id, model, 
                                 track_ocr_confidence, extracted_texts, track_colors, 
                                 reader_instance, confidence_threshold):
        """
        Processes a single regular bounding box detection including OCR and visualization.
        ...
        if should_attempt_ocr:
            ...
            crop_img = RegularBoxProcessor.extract_regular_crop(frame, box)
        ...
        # --- VISUALIZATION ---
        RegularBoxProcessor.draw_bbox_visualization(frame, box, track_id, cls_id, model, track_colors)
    
    @staticmethod
    def draw_bbox_visualization(frame, box, track_id, cls_id, model, track_colors):
        """
        Draws visualization for regular bounding box detection on the frame.
...
def process_frame(frame, model, track_ocr_confidence, extracted_texts, track_colors, reader_instance, tracker_config, confidence_threshold):
    ...
    if has_obb and results[0].obb.id is not None:
        ...
        process_obb_detections(frame, results[0], model, track_ocr_confidence, 
                              extracted_texts, track_colors, reader_instance, confidence_threshold)
    elif has_boxes:
        # Handle regular bounding box format (fallback)
        process_regular_detections(frame, results[0], model, track_ocr_confidence, 
                                  extracted_texts, track_colors, reader_instance, confidence_threshold)

def process_obb_detections(frame, result, model, track_ocr_confidence, extracted_texts, 
                          track_colors, reader_instance, confidence_threshold):
    """
    Processes all OBB detections in a frame.
    
    Args:
        frame (numpy.ndarray): The video frame
        result: YOLO detection result containing OBB data
        model (YOLO): The YOLO model
        track_ocr_confidence (dict): Dictionary storing OCR confidence scores
        extracted_texts (set): Set of extracted texts
        track_colors (dict): Dictionary of track colors
        ...
        confidence_threshold (float): Minimum confidence threshold for OCR success
    """
    obb_coords = result.obb.xyxyxyxy.cpu().numpy()  # 8-point coordinates
    ...
    for obb_points, track_id, cls_id in zip(obb_coords, track_ids, class_ids):
        OBBProcessor.process_obb_detection(
            frame, obb_points, track_id, cls_id, model,
            track_ocr_confidence, extracted_texts, track_colors,
            reader_instance, confidence_threshold
        )

def process_regular_detections(frame, result, model, track_ocr_confidence, extracted_texts, 
                              track_colors, reader_instance, confidence_threshold):
    """
    Processes all regular bounding box detections in a frame.
    
    Args:
        frame (numpy.ndarray): The video frame
        result: YOLO detection result containing box data
        model (YOLO): The YOLO model
        track_ocr_confidence (dict): Dictionary storing OCR confidence scores
        extracted_texts (set): Set of extracted texts
        track_colors (dict): Dictionary of track colors
    ...
    for box, track_id, cls_id in zip(boxes, track_ids, class_ids):
        RegularBoxProcessor.process_regular_detection(
            frame, box, track_id, cls_id, model,
            track_ocr_confidence, extracted_texts, track_colors,
            reader_instance, confidence_threshold
        )
